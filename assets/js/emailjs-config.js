  ...
  window.sendEmailViaEmailJS = function(data, callbacks) {
    ...
    return emailjs.send(EMAILJS_CONFIG.SERVICE_ID, EMAILJS_CONFIG.TEMPLATE_ID, templateParams)
      .then(function(response) {
        if (callbacks && callbacks.onSuccess) callbacks.onSuccess(response);
        return response;
      }, function(error) {
        if (callbacks && callbacks.onError) callbacks.onError('Ошибка отправки: ' + error.text);
